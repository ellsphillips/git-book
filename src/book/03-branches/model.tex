...
If the current production release, say version 1.2, is live with bug that require immediate attention, a \codeinline{hotfix} branch may be created from \codeinline{master}.

For example, say version 1.2 is the current production release running live and causing troubles due to a severe bug. But changes on \codeinline{develop} are yet unstable. We may then branch off a \codeinline{hotfix} branch and start fixing the problem:
